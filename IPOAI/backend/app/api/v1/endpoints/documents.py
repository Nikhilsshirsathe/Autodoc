...
async def upload_document(
    ...
    - `company_id`: Optional company/project ID to associate the document.
...
async def delete_document(document_id: str, user_id: str):
    ...
    try:
        ...
        if doc_res.data.get("uploaded_by") != user_id:
            raise HTTPException(status_code=403, detail="Not authorised to delete this document")

        # Delete storage file
        storage = StorageService()
        try:
            storage.delete_file(doc_res.data["file_path"])
        except Exception as e:
            logger.warning("Storage delete failed (continuing): %s", e)

        # Delete DB records (cascades will handle child rows if set up)
