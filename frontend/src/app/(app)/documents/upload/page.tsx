...
import { linkDocToProject, getDocIdsForProject } from "@/lib/project-doc-map";
...
export default function DocumentUploadPage() {
  ...
  // ── Load documents once user is known ─────────────────────────────────────
      ...
      for (const file of accepted) {
        ...
        try {
          const doc = await uploadDocument(
            file,
            userId,
            selectedProjectId ?? undefined,  // link to active project
            "auto",        // backend auto-detects document type
            true,
            (pct) => {
              setUploading((p) =>
          ...
          // Link to project in localStorage as backup mapping
          if (selectedProjectId) linkDocToProject(doc.id, selectedProjectId);
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {(doc.status === "failed" || doc.status === "completed") && (
                      <Button
                        ...
                        title={doc.status === "failed" ? "Retry processing" : "Reprocess document"}
